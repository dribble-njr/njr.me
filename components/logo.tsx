...
const ChakraPath: FC<{
  ...
  const { d, dashArray, start, duration } = path
  const prefersReducedMotion = usePrefersReducedMotion()

  let animation = prefersReducedMotion
    ? 'none'
    : `${draw} ${duration}s ease forwards ${start}s`
    ...
    <Link href="/" scroll={false}>
      ...
      >
        {paths.map((path, index) => (
          <ChakraPath key={index} path={path} />
        ))}
      </Box>
    </Link>
  )
